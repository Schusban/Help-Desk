        ...
        <v-card-text>
          <v-form ref="form" v-model="valid">
            ...
            <v-select
              ...
              item-value="email"
              v-model="editChamadoData.atribuido_para"
...
export default {
  data() {
    return {
      chamados: [],
      usuarios: [], // Pode ser preenchido se necessário
      search: '',
      dialog: false,
      valid: false,
      editChamadoData: {},
  ...
  methods: {
    ...
    async carregarChamados() {
      try {
        const response = await axios.get('http://127.0.0.1:3333/listar_chamados_usuarios', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
